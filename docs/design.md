...
- **볼드(strong)**: 검정 `#111111` 글자 + **연한 빨강 밴드 `#FAD9D9`** (2026-08-17 초록
  `#C5E8AE` → `#F6C8C8` → 한 단계 더 연하게), **직각 모서리**, **순수 `inline` +
  `box-decoration-break: clone`** — 밴드가 줄 중간에서 자연스럽게 잘려 본문과 같이
  흐른다. **`display: inline-block`은 기각** (2026-08-17 — 원래 "통째로 다음 줄로"
  요청으로 넣었으나, 볼드가 단어→문장 중심으로 바뀌면서 긴 볼드가 줄을 강제로 바꾸고
  여러 줄 밴드가 위아래로 겹쳐서 같은 날 번복. `line-height: 1.25`도 inline-block
  전용이라 함께 제거). **세로 패딩 0 유지** — 있으면 연속 줄 밴드가 겹친다.
  형광색(`#B7FF5E`)·분홍은 시도 후 기각.
...
- **라이트박스** (2026-08-17): `article.prose img` 클릭 → 어두운 오버레이(`rgba(0,0,0,.78)`)에
  원본 크기로 띄운다 (자연 크기 이상 업스케일 안 함 — `max-width`만 제한). 클릭/Esc로 닫기.
  외부 라이브러리 없음 — PostLayout의 인라인 바닐라 스크립트. 본문 사진이 480px로
  전처리되므로 라이트박스에서도 최대 480px — 더 크게 원하면 prep-image `-Size`를 키운다.
...
## 글 끝 이전/다음/돌아가기 (2026-09-09)

- `src/components/PostNav.astro` — 글 본문이 끝나는 자리(`<slot />` 바로 아래)에 항상 붙는다.
  왼쪽 "← 이전 글"(더 오래된 글), 오른쪽 "다음 글 →"(더 최근 글), 가운데 아래 "전체 글로 돌아가기".
  양 끝 글은 없는 쪽을 비운다. 화면 하단 고정 바 안은 논의 후 기각 — 상단 스티키 바와 겹치고
  모바일 세로 공간을 먹는다 (2026-09-09 사용자 선택).
- **데이터는 빌드 타임에 계산한다** — `src/lib/postNav.ts`의 `postNav()`를 `[slug].astro`의
  `getStaticPaths`가 부르고 PostLayout 경유로 넘긴다. getStaticPaths는 import 말고는 바깥
  스코프를 못 보므로 로직을 lib로 뺐다. **JS가 한 줄도 없다** — 순수 `<a>`라 ViewTransitions
  재진입 때 바인딩이 빠지는 문제(갈래 탭 사고)가 원리적으로 없다.
- **이전/다음은 같은 갈래(회고·일기·생각) 안에서만 잇는다** — 기준은 탭과 같은 `tags.includes`.
  전체 순서로 이으면 일기 다음이 회고로 튄다. 갈래 태그가 없는 글은 전체 순서 + `/blog/all/`로 폴백.
  정렬은 pubDate 오름차순, **같은 날은 파일명(slug)순** — pubDate가 날짜뿐이라 같은 날 글이 흔하다
  (8/25에 3편). 이 2차 정렬을 빼면 빌드마다 순서가 흔들릴 수 있다.
- 돌아가기는 `/blog/#일기`처럼 **갈래 해시를 붙인다** — `/blog/`의 탭 초기화가 `location.hash`를
  읽어 그 탭을 연다(실측: 해시가 퍼센트 인코딩돼 와도 `decodeURIComponent`로 풀린다, `history.state` 유지).
- 스타일: 카드 테두리 없이 `divider` 선 하나, 라벨은 `.font-pixel`(초록), 제목은 본문색 볼드
  `line-clamp-2`, 호버 시 primary. 돌아가기는 푸터 링크와 같은 `link link-hover font-pixel`.
  **sm(640px) 미만은 세로로 쌓는다** — 반폭에 한글 제목을 넣으면 네 줄까지 늘어난다
  (실측 386px: 가로 넘침 없음).
