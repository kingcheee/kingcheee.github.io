...
- **볼드(strong)**: 검정 `#111111` 글자 + **연한 빨강 밴드 `#FAD9D9`** (2026-08-17 초록
  `#C5E8AE` → `#F6C8C8` → 한 단계 더 연하게), **직각 모서리**, **`display: inline-block`** — 볼드는 줄 중간에서
  잘리지 않고 통째로 다음 줄로 내려간다 (2026-08-17 사용자 요청). 줄 폭보다 긴 볼드만
  내부 줄바꿈하며, 그때를 위해 `box-decoration-break: clone`(줄바꿈 시 밴드 유지)을 남겨둔다.
  **inline-block에는 `line-height: 1.25`가 필수** — 없으면 밴드 높이가 본문 줄 높이를
  따라가 두꺼워진다 (실측 반려, 2026-08-17). 세로 패딩도 0으로. 형광색(`#B7FF5E`)·분홍은
  시도 후 기각.
